...
A `.pb.go` sits in an ordinary source path, so paths cannot catch it — but every generator
disowns its output in the file's head, and that does. The first 40 lines are checked for the banners
generators actually emit — `@generated`, `<auto-generated>`, `code generated by`, `autogenerated
by`, `generated code … do not modify`, `this file is/was generated`, `generated by … DO NOT EDIT`,
and protobuf's own. That covers protoc, Dart's build_runner, .NET designers, Thrift, SWIG and the
Meta convention.

**Every pattern names the file or the generator.** A bare `auto-generated` is ordinary prose —
*"tokens are auto-generated per session"* — and skipping on it silently disarms the hook for a
real file. A false positive you notice; a silent skip you never do.

Read from disk, not from the edit — a `.pb.go` is skipped on the strength of a header the edit
itself never touches, and the diff path carries only added lines.

A bare `DO NOT EDIT` is likewise not enough: hand-written files use it to warn a human.
